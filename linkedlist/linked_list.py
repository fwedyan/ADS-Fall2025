class LinkedList:
    ...
    # PART ONE INSERTION, a stack can benifit from
    def insert_first(self, e):
        newest = self.Node(e, None)
        # in case if the list is empty which is 
        #going to happen when it is the first 
        #element
        if self.is_empty(): #if not self._head
            self._head = newest
            self._tail = newest
